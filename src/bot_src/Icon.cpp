...
int Icon::getBytesPerPacket(){
    ...
    if(this->strategicPacketNumber(this->m_currentPacket) == this->m_numberOfPackets - 1){
        // the last packet - this has the possibility of containing less rows
        // rows remaining * bytes per row
       
        return (this->getIconImageSize() - (this->strategicPacketNumber(this->m_currentPacket) * floor(900 / ((this->m_width - 1) * 2))) - 1) * (this->getIconImageSize() * 2);
    }
